...
windowDisplay = pygame.display.set_mode(WINDOW_SIZE)
...
while running:
    ...
    if not won:
        ...
        pygame.draw.circle(display, player.color, (player.position.x, player.position.y),player.radius)
    ...
    s.set_alpha(levels[level].alpha)         
    s.fill(levels[level].theme)
    display.blit(s, (0,0))
    # /PP
    windowDisplay.blit(display, (0,0))
    pygame.display.flip()
    display.fill((5,9,5))
    time+=1
