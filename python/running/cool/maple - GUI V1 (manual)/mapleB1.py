...
###### SCENE WINDOW ###################################################################################
	...
	for x in range(0, len(project.areas)):
		...
		sceneS.blit(font16.render(visibility, 20, col), (monitor_size[0]*1/5-(vTextWidth), y+8))
		y += 30
		...
		for l in range(0, len(project.areas[x].layers)):
			...
			if project.areas[x].editIsExpanded:
				...
				if pressed and lhitbox.collidepoint( (mousePos[0] , mousePos[1])) and mousePos[1] > 49:
					selectedLayer = l
					selectedArea = x
