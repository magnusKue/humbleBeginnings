...
pygame.event.set_allowed([QUIT, KEYDOWN, KEYUP, MOUSEWHEEL])
...
def updateTiles(selectedObjectKind, editedTiletype, editedObjecttype, optionsRequired, optionsKind, editingMode, editedTileId, tileAdd, addTileAnimated, tileScroll):
	...
	if selectedObjectKind == 1:
		...
		### Render Tile Icons ##########
			try:
				icon = pygame.image.load("tiles/"+project.tileTypes[Id].sprite).convert_alpha()
			except:
				try:
					icon = pygame.image.load("tiles/"+str(project.tileTypes[Id].get_frame(time))).convert_alpha()
				except:
...
###### options WINDOW ###################################################################################
	...
	if optionsRequired:
		if optionsKind == 1:
			...
			############ custom collision############################## pragma custom collision
			...
			for corner in tileAdd.collisionRect:
				pygame.draw.circle(optionsS, (255,50,50), (10 + int(corner[0]*iconSize), (optionsS.get_height()-10-iconSize)+ int(corner[1])*iconSize), 5)
			pygame.draw.rect(optionsS, (255, 50, 50), Rect( (10 + int(tileAdd.collisionRect[0][0]*iconSize), (optionsS.get_height()-10-iconSize) + int(tileAdd.collisionRect[0][1])*iconSize), (int(tileAdd.collisionRect[1][0]*iconSize), int(tileAdd.collisionRect[1][1]*iconSize)) ), 2, 2 ) 
