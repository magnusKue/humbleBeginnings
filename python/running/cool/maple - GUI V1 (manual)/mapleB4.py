...
def updateTiles(selectedObjectKind, editedTiletype, editedObjecttype, optionsRequired, optionsKind):
	...
	if not optionsRequired and objBox.collidepoint((int(mousePos[0]-monitor_size[0]*4/5), int(mousePos[1]-monitor_size[1]*2/5))) and pressed:
		selectedObjectKind = 0

	tilesBox = Rect((int(tilesS.get_width()*1/5+21),11), (int(tilesS.get_width()*1/5), int(tilesS.get_height()*1/17-10)))
	if not optionsRequired and tilesBox.collidepoint((int(mousePos[0]-monitor_size[0]*4/5), int(mousePos[1]-monitor_size[1]*2/5))) and pressed:
	...
	col = (141, 153, 174)
	if not optionsRequired and pressed and addTileHitbox.collidepoint((mousePos[0] - previewS.get_width(), mousePos[1] - sceneS.get_height())):
		col = (70, 75, 90)
		if selectedObjectKind == 1:
...
while running:
	...
	debugger.debug(rpressed)
	debugger.debug(pressed)
